...
| EIA STEO forecast | 1.033 | 0.648 | 1.532 | — |
...
- per-unit injection, withdrawal and holding costs;
...
| Fixed 50/50 ARIMAX-EIA path | $103,946 |
...
For each 2017–2025 forecast year, the notebook optimises a schedule using the forecast available at that date, freezes every decision and revalues the unchanged schedule using later realised monthly prices.
...
The selected model's lower pooled price error does not translate into consistently useful storage timing. EIA is also inconsistent, and its median realised value is zero, but its no-action years avoid operating costs when the published curve contains no sufficient spread.
...
The notebook also plots the cumulative sum of the annual realised values to show how the separate outcomes develop across forecast years. This is an illustrative running sum rather than a common-date portfolio valuation.
...
- evaluation of a frozen schedule without re-optimisation;
- preservation of non-zero opening inventory; and
